...
% LOADKSDIR Load Kilosort spike sorting output directory
%
%   spikeStruct = loadKSdir(ksDir)
%   spikeStruct = loadKSdir(ksDir, params)
%
%   Reads NPY files from a Kilosort output directory and returns a struct
%   with all spike sorting data.
%
%   Parameters:
%     ksDir  - Path to Kilosort output directory
%     params - (Optional) Struct with fields:
%                params.excludeNoise - Exclude noise clusters (default: true)
%                params.loadPCs      - Load PC features (default: true)
%
%   Returns:
%     spikeStruct - Struct with fields:
%       st              - Spike times in seconds
%       spikeTemplates  - Template assignment for each spike (0-indexed)
%       clu             - Cluster assignment for each spike
%       tempScalingAmps - Template scaling amplitudes
%       cgs             - Cluster groups (0=noise, 1=mua, 2=good, 3=unsorted)
%       cids            - Cluster IDs
%       xcoords         - Channel x-coordinates
%       ycoords         - Channel y-coordinates
%       temps           - Template waveforms
%       winv            - Whitening matrix inverse
%       pcFeat          - PC features [nSpikes x nFeatures x nLocalChannels]
%       pcFeatInd       - PC feature channel indices [nTemplates x nLocalChannels]
%       chan_map         - Channel map
%       sample_rate     - Sampling rate from params.py
%
%   See also: readNPY, readClusterGroupsCSV
...
chan_map = readNPY(fullfile(ksDir, 'channel_map.npy'));
